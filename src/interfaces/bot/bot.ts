...
`;
  ...
  // Admin commands (TODO: add admin check)
  bot.command('admin_draw', async (ctx) => {
    // TODO: Add admin authorization
    const result = await deps.drawRaffleUseCase.execute();
    if (result.status === 'success') {
      await ctx.reply(`✅ Розыгрыш завершен! Победитель: ${result.winnerUserId}`);
    } else {
      await ctx.reply(`❌ Не удалось провести розыгрыш: ${result.status}`);
    }
  });
...
`;
      ...
      case 'ready':
        ...
        statusText = `
🎰 *Розыгрыш #${r.id} готов к проведению*
